...
var _ datasource.DataSourceWithConfigure = (*PlansDataSource)(nil)
...
func (d *PlansDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
  ...
  env := PlansDataListDataSourceEnvelope{}
  maxItems := int(data.MaxItems.ValueInt64())
  acc := []attr.Value{}
  if maxItems <= 0 {
    maxItems = 1000
  }
  page, err := d.client.Plans.List(ctx, params)
  if err != nil {
  ...
  result, diags := customfield.NewObjectListFromAttributes[PlansItemsDataSourceModel](ctx, acc)
  resp.Diagnostics.Append(diags...)
  data.Items = result

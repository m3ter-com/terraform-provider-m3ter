...
var _ datasource.DataSourceWithConfigure = (*CounterDataSource)(nil)

func NewCounterDataSource() datasource.DataSource {
	return &CounterDataSource{}
}

func (d *CounterDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_counter"
}

func (d *CounterDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *CounterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if data.FindOneBy == nil {
		...
		page, err := d.client.Counters.List(
			ctx,
			data.FindOneBy.OrgID.ValueString(),
			params,
		)
		if err != nil {
			resp.Diagnostics.AddError("failed to make http request", err.Error())

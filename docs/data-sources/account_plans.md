...
description: |-
  ...
  AccountPlan Active/Inactive. Set start and end dates to define the period the AccountPlan is active for the Account.AccountPlan per Product. If an end customer consumes multiple Products, create separate AccountPlans to charge for each Product.
  AccountPlan Constraints:
  Only one AccountPlan per Product can be active at any one time for an Account.If you create a Plan as a custom Plan for a specific Account, you can only use it to create an AccountPlan for that Account.
  AccountPlanGroups
  ...
  The new Plan Group contains a Product Plan that also belongs to a Plan Group already attached to the Account.The dates for these "matched Plan" Plan Groups being active for the Account would overlap.Then you'll receive an error and the attachment will be blocked.
---
